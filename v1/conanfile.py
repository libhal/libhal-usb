...
class libhal_usb_conan(ConanFile):
    name = "libhal-usb"
    license = "Apache-2.0"
    url = "https://github.com/libhal/libhal-usb"
    homepage = "https://libhal.github.io/libhal-usb"
    description = ("A USB device enumerator and set of USB protocol "
                   "utilities for embedded systems using modern C++")
    topics = ("usb", "embedded", "hardware", "hal")
    ...
    def _compilers_minimum_version(self):
        return {
            "gcc": ("14", "GCC 14+ required for libhal-usb"),
            "clang": (
                "19",
                "Clang 19+ required for libhal-usb"
            ),
            "apple-clang": (
                "19.0.0",
                "Apple Clang 19+ for libhal-usb"
            ),
            "msvc": (
                "193.4",
                "MSVC 14.34+ (Visual Studio 17.4+) for libhal-usb"
            )
        }
